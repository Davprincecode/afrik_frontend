...
function Vlog() {
  ...
  // Re-process Instagram embeds after data loads
  useEffect(() => {
    if ((window as any).instgrm) {
      (window as any).instgrm.Embeds.process();
    }
  ...
  return (
    <div className="vlog">
        ...
        {data.map((item, index) => (
          <div className="vlogCon" key={index}>
            ...
            <div className="vlogContent">
              <div className="vlogHeading">{item.title}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="vlogBottom">
        <a
          href="https://www.instagram.com/temiloveafrik/?utm_source=ig_embed&ig_rid=63500371-fa37-4d0a-a491-46a4f509cd41&ig_mid=7BAD6E23-5A2C-473D-9ED4-BB17FACF703D"
          target="_blank"
